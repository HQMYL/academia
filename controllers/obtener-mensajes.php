...
$outgoing_id = $_SESSION['emisor_id'];;
$incoming_id = $_SESSION['receptor_id'];
$id_propuesta = $_SESSION['id_propuesta'];
  $messages = [];
  $sql = "SELECT * FROM mensajes LEFT JOIN users ON users.id_usuario = mensajes.incoming_msg_id
                WHERE (outgoing_msg_id = {$outgoing_id} AND incoming_msg_id = {$incoming_id})
                OR (outgoing_msg_id = {$incoming_id} AND incoming_msg_id = {$outgoing_id}) AND id_propuesta = {$id_propuesta} ORDER BY id_mensaje DESC";
